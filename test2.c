...
float * create_H_matrix(float H[]) {
    ...
    for (int i = 0; i < 600; i++) {
        for (int j = 0; j < 256; j++) {
            H[i*256+j] = S[plaintexts[i] ^ j];
            H[i*256+j] = Hamming_weight(H[i*256+j]);
        }
    }
